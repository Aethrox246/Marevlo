...
async def lifespan(app: FastAPI):
    ...
    # Bring up MIRA (Claude client, retriever, Qdrant, quota backend, ...).
    # Best-effort: a failure here logs but doesn't kill the rest of the app.
    try:
        await init_mira_on_startup(app)
    except Exception as exc:
        logger.warning("mira_startup_failed err=%s", exc)
    try:
        yield
    finally:
