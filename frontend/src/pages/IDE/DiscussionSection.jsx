    ...
    return (
        <div style={{ position: 'relative', margin: '8px 0', borderRadius: 8, overflow: 'hidden', border: '1px solid var(--color-border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 12px', background: 'var(--color-app-bg)', borderBottom: '1px solid var(--color-border)' }}>
                ...
                <button onClick={copy} style={{ background: 'none', border: 'none', cursor: 'pointer', color: copied ? '#10b981' : 'var(--color-muted-text)', padding: '2px 6px', borderRadius: 5, fontSize: 10, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
...
function ContentRenderer({ text, collapsed, onExpand }) {
    ...
    return (
        <div>
            ...
            {collapsed && isLong && (
                <button onClick={onExpand} style={{ fontSize: 11, fontWeight: 700, color: '#6366f1', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 0', marginTop: 2 }}>
                    Read more \u2193
                </button>
            )}
        </div>
...
/* ─── Tags ─── */
const TAGS = [
    { id: 'question',    label: '\u2753 Question',    color: '#3b82f6', bg: 'rgba(59,130,246,0.1)'  },
    { id: 'approach',    label: '\u{1F9E0} Approach',    color: '#8b5cf6', bg: 'rgba(139,92,246,0.1)'  },
    { id: 'walkthrough', label: '\u{1F4D6} Walkthrough', color: '#10b981', bg: 'rgba(16,185,129,0.1)'  },
    { id: 'bug',         label: '\u{1F41B} Bug',          color: '#ef4444', bg: 'rgba(239,68,68,0.1)'   },
];
        ...
        <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4, flexWrap: 'wrap' }}>
                    ...
                    {reply.isAccepted && (
                        <span style={{ fontSize: 9, fontWeight: 800, color: '#10b981', background: 'rgba(16,185,129,0.12)', padding: '2px 7px', borderRadius: 20, border: '1px solid rgba(16,185,129,0.25)', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                            <CheckCircle size={9} fill="#10b981" /> Accepted
                        </span>
                    )}
                </div>
                ...
                <div style={{ display: 'flex', gap: 5, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <button onClick={() => onUpvote(reply.id)} aria-pressed={reply.isUpvoted} style={{
                        ...
                        border: `1px solid ${reply.isUpvoted ? '#6366f1' : 'var(--color-border)'}`,
                        background: reply.isUpvoted ? 'rgba(99,102,241,0.12)' : 'transparent',
                        color: reply.isUpvoted ? '#6366f1' : 'var(--color-muted-text)',
                    ...
                    {(canAccept || reply.isAccepted) && (
                        <button onClick={() => canAccept && onAccept(reply.id)} title={canAccept ? 'Mark as accepted answer' : 'Accepted answer'} style={{
                            display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 8,
                            border: `1px solid ${reply.isAccepted ? '#10b981' : 'var(--color-border)'}`,
                            background: reply.isAccepted ? 'rgba(16,185,129,0.1)' : 'transparent',
                            color: reply.isAccepted ? '#10b981' : 'var(--color-muted-text)',
            ...
            {isOwn && hovered && (
                <button onClick={() => onDelete(reply.id)} title="Delete reply" style={{
                    ...
                    borderRadius: 6, color: '#ef4444', opacity: 0.75, flexShrink: 0, alignSelf: 'flex-start',
                }}
            ...
            <div style={{ display: 'flex', gap: 10, padding: '14px 16px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flexShrink: 0, width: 32, paddingTop: 2 }}>
                    <button onClick={() => onUpvote(post.id)} aria-pressed={post.isUpvoted} title="Upvote" style={{
                        ...
                        background: post.isUpvoted ? 'rgba(99,102,241,0.15)' : 'var(--color-surface-hover)',
                        color: post.isUpvoted ? '#6366f1' : 'var(--color-muted-text)',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        cursor: 'pointer', transition: 'all 0.18s',
                        transform: post.isUpvoted ? 'scale(1.12)' : 'scale(1)',
                    }}
                        onMouseEnter={e => { if (!post.isUpvoted) e.currentTarget.style.background = 'rgba(99,102,241,0.1)'; }}
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: post.isUpvoted ? '#6366f1' : 'var(--color-muted-text)', lineHeight: 1 }}>
                        {post.upvotes || 0}
                    </span>
                </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                            ...
                            {post.isSpoiler && !revealed && <span style={{ fontSize: 9, fontWeight: 700, color: '#f59e0b' }}>🚫 Spoiler</span>}
                            {post.tag && <TagBadge tagId={post.tag} small />}
                            {post.isPinned && (
                                <span style={{ fontSize: 9, fontWeight: 800, color: '#f59e0b', background: 'rgba(245,158,11,0.12)', padding: '2px 7px', borderRadius: 20, border: '1px solid rgba(245,158,11,0.25)' }}>
                                    📌 PINNED
                                </span>
                            )}
                            {hasAccepted && (
                                <span style={{ fontSize: 9, fontWeight: 800, color: '#10b981', background: 'rgba(16,185,129,0.1)', padding: '2px 7px', borderRadius: 20, border: '1px solid rgba(16,185,129,0.25)', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                                    <CheckCircle size={9} fill="#10b981" /> Solved
                                </span>
                            )}
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                            <button onClick={handleShare} title="Copy link to post" style={{
                                ...
                                color: copied ? '#10b981' : 'var(--color-muted-text)', opacity: 0.65,
                            }}
                                onMouseEnter={e => e.currentTarget.style.opacity = '1'}
                    ...
                    {editing ? (
                        <div style={{ marginBottom: 10 }}>
                            <textarea
                                ...
                                rows={4}
                                style={{
                                    width: '100%', padding: '9px 12px', borderRadius: 10, border: '1px solid #6366f1',
                            ...
                            <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', marginTop: 6 }}>
                                ...
                                <button onClick={handleSaveEdit} disabled={editSaving || !editText.trim()} style={{
                                    ...
                                    background: editText.trim() ? '#6366f1' : 'var(--color-surface-hover)',
                        ...
                        <div style={{ position: 'relative', marginBottom: 10, borderRadius: 8, overflow: 'hidden' }}>
                            ...
                            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <button onClick={() => setRevealed(true)} style={{
                                    ...
                                    background: 'rgba(99,102,241,0.85)', color: '#fff', fontWeight: 700, fontSize: 12, cursor: 'pointer',
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <button onClick={() => { setReplyOpen(o => !o); if (!replyOpen) setShowReplies(true); }} style={{
                            ...
                            border: `1px solid ${replyOpen ? '#3b82f6' : 'var(--color-border)'}`,
                            background: replyOpen ? 'rgba(59,130,246,0.1)' : 'transparent',
                            color: replyOpen ? '#3b82f6' : 'var(--color-muted-text)',
                        ...
                        <div style={{ marginTop: 12, display: 'flex', gap: 8, alignItems: 'flex-start', animation: 'dsSlideDown 0.2s ease' }}>
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 }}>
                                    <span style={{ fontSize: 10, color: replyText.length > MAX_REPLY_LENGTH * 0.85 ? '#ef4444' : 'var(--color-muted-text)' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 6 }}>
                                        ...
                                        <button onClick={submitReply} disabled={!replyText.trim() || submitting} style={{
                                            padding: '5px 14px', borderRadius: 7, border: 'none',
                                            background: replyText.trim() ? '#3b82f6' : 'var(--color-surface-hover)',
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{
                        ...
                        background: user ? '#10b981' : 'var(--color-border)',
                        boxShadow: user ? '0 0 0 2px rgba(16,185,129,0.2)' : 'none', flexShrink: 0,
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--color-border)', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <button onClick={() => setComposeSpoiler(s => !s)} style={{
                        padding: '3px 10px', borderRadius: 20, fontSize: 10, fontWeight: 700,
                        border: `1px solid ${composeSpoiler ? '#f59e0b' : 'var(--color-border)'}`,
                        background: composeSpoiler ? 'rgba(245,158,11,0.12)' : 'transparent',
                        color: composeSpoiler ? '#f59e0b' : 'var(--color-muted-text)',
                    ...
                    <div style={{ flex: 1, position: 'relative' }}>
                        ...
                        <textarea
                            ...
                            style={{
                                ...
                                border: `1px solid ${overLimit ? '#ef4444' : 'var(--color-border)'}`,
                            ...
                            onFocus={e => { if (user) e.currentTarget.style.borderColor = '#6366f1'; }}
                            onBlur={e => { e.currentTarget.style.borderColor = overLimit ? '#ef4444' : 'var(--color-border)'; }}
                        />
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 7 }}>
                            <span style={{ fontSize: 10, color: overLimit ? '#ef4444' : 'var(--color-muted-text)' }}>
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    background: canPost ? 'linear-gradient(135deg,#6366f1,#4f46e5)' : 'var(--color-surface-hover)',
            ...
            <div style={{
                ...
                {[
                    ...
                    <button key={s.id} onClick={() => setSortBy(s.id)} style={{
                        padding: '3px 10px', borderRadius: 20, fontSize: 10, fontWeight: 700,
                        border: `1px solid ${sortBy === s.id ? '#6366f1' : 'var(--color-border)'}`,
                        background: sortBy === s.id ? 'rgba(99,102,241,0.12)' : 'transparent',
                        color: sortBy === s.id ? '#6366f1' : 'var(--color-muted-text)',
