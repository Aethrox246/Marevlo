...
export default function Feed({ setView }) {
    ...
    const handleLike = (postId) => {
        setPosts(prev => prev.map(post => {
            if (post.id === postId) {
                ...
                return { ...post, likedByMe: isLiked, likes: isLiked ? post.likes + 1 : post.likes - 1 };
            }
            return post;
        }));
    };
    ...
    const handleRepost = (post) => {
        const repost = {
            ...
            likedByMe: false, commentsList: []
        };
        setPosts(prev => {
            const updatedPosts = prev.map(p => p.id === post.id ? { ...p, reposts: (p.reposts || 0) + 1 } : p);
        ...
        setPosts(prev => prev.map(post => {
            if (post.id === postId) {
                return {
                    ...
                    commentsList: [
                        ...(post.commentsList || []),
                        { id: Date.now(), author: user ? user.name : "User", content: commentText, time: "Just now" }
                    ]
                };
            }
            return post;
    ...
    const handlePublishArticle = () => {
        ...
        setPosts(prev => [{
            id: Date.now(), author: user ? user.name : "User", role: "Developer",
            avatar: user ? user.name[0].toUpperCase() : "U", title: articleTitle,
            content: articleContent, image: articleImage, likes: 0, comments: 0,
            time: "Just now", isArticle: true, likedByMe: false, commentsList: []
        }, ...prev]);
        setArticleTitle(""); setArticleContent(""); setArticleImage(null); setIsArticleModalOpen(false);
    ...
    const handleCreateEvent = () => {
        ...
        setPosts(prev => [{
            id: Date.now(), author: user ? user.name : "User", role: "Developer",
            avatar: user ? user.name[0].toUpperCase() : "U", content: eventDescription,
            eventDetails: { title: eventTitle, date: eventDate, location: eventLocation },
            likes: 0, comments: 0, time: "Just now", isEvent: true, likedByMe: false, commentsList: []
        }, ...prev]);
        setEventTitle(""); setEventDate(""); setEventLocation(""); setEventDescription(""); setIsEventModalOpen(false);
                    ...
                    <aside className="hidden lg:flex flex-col gap-5 h-fit sticky top-24">
                        ...
                        <div className="rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl" style={{
                            ...
                            <div className="p-5 sm:p-6">
                                ...
                                {[
                                    { initials: 'AK', name: 'Arjun Kumar', role: 'Full Stack Dev', grad: 'linear-gradient(135deg,#6366f1,#8b5cf6)' },
                                    { initials: 'SG', name: 'Sneha Gupta', role: 'ML Engineer', grad: 'linear-gradient(135deg,#f43f5e,#ec4899)' },
                                    { initials: 'RM', name: 'Rahul Mehta', role: 'DSA Coach', grad: 'linear-gradient(135deg,#06b6d4,#0ea5e9)' },
                                ].map(({ initials, name, role, grad }) => (
                                    <div key={initials} className="flex items-center justify-between mb-3 last:mb-0">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p style={{ fontSize: '0.82rem', fontWeight: 600, color: 'var(--color-primary-text)' }}>{name}</p>
                                                <p style={{ fontSize: '0.7rem', color: 'var(--color-muted-text)' }}>{role}</p>
                                            </div>
                                        </div>
                                        <button style={{
                                            ...
                                            onMouseEnter={e => { e.currentTarget.style.background = 'linear-gradient(135deg,#6366f1,#8b5cf6)'; e.currentTarget.style.color = '#fff'; e.currentTarget.style.border = '1.5px solid transparent'; }}
                                            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#6366f1'; e.currentTarget.style.border = '1.5px solid rgba(99,102,241,0.5)'; }}
                                        >
                                            Follow
                                        </button>
                                    </div>
