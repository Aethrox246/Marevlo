...
function BFSVisualizer() {
    ...
    return (
        <div className="lg:col-span-5 rounded-3xl border overflow-hidden relative"
            style={{ background: '#0a0a1a', borderColor: 'rgba(6,182,212,0.35)' }}>
...
function AITutorCard() {
    ...
    return (
        <div className="lg:col-span-3 rounded-3xl border overflow-hidden relative transition-all duration-500"
            style={{ background: '#0f0f23', borderColor: isPremium ? 'rgba(139,92,246,0.35)' : 'rgba(6,182,212,0.3)' }}>
            <style>{`
            ...
            <div className="p-5 relative z-10 flex flex-col h-full">
                ...
                {/* Level Tabs */}
                <div className="flex gap-1 mb-4 p-1 rounded-lg bg-black/20 border border-white/5 transition-colors relative overflow-hidden">
                    {/* Highlight Pill animate block */}
                    <div className="absolute top-1 bottom-1 rounded-md transition-all duration-300"
                        style={{ width: '33.33%', left: `${levelIdx * 33.33}%`, background: 'rgba(255,255,255,0.1)' }} />
                    {AI_LEVELS.map((lvl, i) => (
                        <button key={lvl.id} onClick={() => { if (i !== levelIdx) { setLevelIdx(i); setIsThinking(true); setTimeout(() => setIsThinking(false), 800); } }}
                            className={`flex-1 text-[9px] font-bold py-1 transition-all cursor-pointer z-10 ${i === levelIdx ? 'text-white' : 'text-neutral-500 hover:text-neutral-300'}`}>
                            {lvl.label}
                        </button>
                    ))}
                </div>
...
function ApproachesCard() {
    ...
    return (
        <div className="lg:col-span-5 rounded-3xl relative overflow-hidden"
            style={{ background: '#0a0a1a', border: '1px solid rgba(255,255,255,0.07)' }}>
...
function LaddersCard() {
    ...
    return (
        <div className="lg:col-span-7 rounded-3xl relative overflow-hidden"
            style={{ background: '#080814', border: '1px solid rgba(255,255,255,0.07)' }}>
...
export default function LandingPage({ onStart, onExplore }) {
    ...
    return (
        <div className="overflow-y-auto h-full text-primary-text scroll-smooth bg-app-bg">
            ...
            {/* ── HERO ────────────────────────────────────────────────────────── */}
            <section className="relative pt-32 pb-36 px-4 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px]">
                ...
                <div className="max-w-7xl mx-auto px-4 relative">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 grid-rows-auto gap-4">
                        ...
                        <div className="lg:col-span-4 rounded-3xl border overflow-hidden relative group hover:scale-[1.01] transition-transform duration-300"
                            style={{ background: '#0f0f23', borderColor: 'rgba(99,102,241,0.3)' }}>
                        ...
                        <div className="lg:col-span-12 rounded-[2rem] border overflow-hidden relative group hover:scale-[1.01] transition-transform duration-500 shadow-2xl"
                            style={{ background: 'linear-gradient(145deg, #090914, #12122a)', borderColor: 'rgba(99,102,241,0.5)', boxShadow: '0 20px 40px -10px rgba(99,102,241,0.15)' }}>
